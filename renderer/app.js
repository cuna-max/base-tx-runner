...
function updateStatus(planned, sent, confirmed, failed) {
  ...
  const progress = planned > 0 ? (sent / planned) * 100 : 0;
  $("progress-fill").style.width = `${progress}%`;
  $("progress-text").textContent = `전송됨: ${sent} / 총: ${planned}`;
}
...
async function runTransactionLoop() {
  ...
  while (currentIndex < txCount && !shouldStop) {
    ...
    try {
      ...
      if (currentIndex < txCount && delay > 0) {
  ...
  if (shouldStop) {
    ...
    showError(`완료: ${stats.confirmed}개 성공, ${stats.failed}개 실패`);
  }
}
